...
int main(int argc, char *argv[]) {
    ...
    printf("Inseriu:%d threads.\n", nThreads);
    sleep(1);
    
    //Array com os tids de todos as threads
    ...
    subgenom = (int) gl->n_genomes / nThreads;
    ...
    //variavel para nos deslocarmos na lista de genomas
    int j = 0;
    for (int i = 0; i < nThreads; ++i) {
        GENOME *aux = g;
        //criacao de threads e passagem de argumentos as funções
        pthread_create(&thread_id[i], NULL, &compare, (void *) aux);
        pthread_create(&thread_, NULL, &percent, NULL);
        while (j < subgenom) {
            g = g->pnext;
            j++;
        }
    ...
    //espera pelo termino de todos os threads
    for (int i = 0; i < nThreads; ++i) {
...
/**
 * Função responsavel por imprimir a percentagem comparada
 * @return
 */
void *percent() {
    while (1) {
        printf("%.2f%% = %d genomas comparados.\n", (float) count / TOTAL * 100, count);
        //sleep(1);
    }
}

/**
 * Função responsavel pela comparação de genomas
 * @param arg Recebe por argumento o array de mutações
 * @return
 */
void *compare(void *arg) {
    ...
    sprintf(str, "threads_%lu.txt", (long) pthread_self());
    strcat(output, str);
    ...
    //variavel usada para nos deslocarmos na lista de genomas
    int j = 0;
    while (j != subgenom) {
        genome_cmp(g, mutation_array);
        
        //fecho do trinco para uso da zona critica
        pthread_mutex_lock(&count_mutex);
        count++;
        //abertura do trinco, libertação da zona critica
        pthread_mutex_unlock(&count_mutex);
        
        //Escrita para ficheiro dos resultados da comparação
 ...
 */
void save_mutation_array(long pthread, MUTATION_ARRAY *mutation_array, char *path, int detail) {
    ...
    for (int i = 0; i < mutation_array->n_mutations; i++) {
        ...
        sprintf(buf, "TID:%lu;%s;%s|%s|(%d)", pthread, aux->genome_a, aux->genome_b, aux->gene,
                aux->seq_mutations.n);
        if (detail) {
